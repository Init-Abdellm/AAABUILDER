...
// Import enhanced TypeScript provider system
const { enhancedRegistry } = require('../providers/typescript-bridge');
...
class AgentOrchestrator {
  constructor() {
    ...
    // Enhanced TypeScript provider system
    this.enhancedRegistry = enhancedRegistry;
    this.enhancedProvidersInitialized = false;
  }
  ...
  async execute(ast, input = {}, options = {}) {
    ...
    // Initialize enhanced providers if needed
  ...
  async initializeEnhancedProviders() {
    if (this.enhancedProvidersInitialized) return;
    
    try {
      console.debug('Initializing enhanced TypeScript providers...');
      await this.enhancedRegistry.initialize();
      this.enhancedProvidersInitialized = true;
      console.debug('Enhanced providers initialized successfully');
    } catch (error) {
      console.warn(`Failed to initialize enhanced providers: ${error.message}`);
      // Continue with basic providers if enhanced ones fail
    }
  }
  ...
  async resolveVariables(ast, context) {
    ...
    for (const [name, varDef] of Object.entries(ast.vars)) {
      // Support both schemas: { type:path/value } and { from: input|env|literal }
      const source = varDef.from || varDef.type;
      if (source === 'input') {
        const path = varDef.path || name; // default to input.name
        context.vars[name] = this.getNestedValue(context.input, path);
        console.debug(`Variable '${name}' resolved from input path '${path}': ${context.vars[name]}`);
      } else if (source === 'env') {
        const envKey = varDef.path || varDef.value || name.toUpperCase();
        context.vars[name] = process.env[envKey];
        console.debug(`Variable '${name}' resolved from env '${envKey}': ${context.vars[name]}`);
      } else if (source === 'literal' || source === 'string' || source === 'number' || source === 'boolean') {
        context.vars[name] = varDef.value;
        console.debug(`Variable '${name}' set to literal: ${context.vars[name]}`);
      } else {
        // Fallback: pass through provided value
        context.vars[name] = varDef.value !== undefined ? varDef.value : undefined;
        console.debug(`Variable '${name}' used fallback value: ${context.vars[name]}`);
      }
    }
  }
  ...
  async executeLLMStep(step, context) {
    ...
    logger.debug(`Provider returned: ${typeof result}, value: ${result ? (typeof result === 'string' ? result.substring(0, 100) : JSON.stringify(result).substring(0, 100)) : 'undefined'}`);

    // Normalize to plain string where possible
  ...
  async resolveOutput(ast, context) {
    // If structured outputs are not defined, support single output (new-format parser sets ast.output)
    if (!ast.outputs) {
      if (ast.output) {
        try {
          // If output is a bare variable name, wrap with braces to resolve
          const template = /\{.*\}/.test(ast.output) ? ast.output : `{${ast.output}}`;
          const rendered = await renderer.render(template, context);
          return rendered;
        } catch (error) {
          logger.warn(`Failed to render single output: ${error.message}`);
          return ast.output;
        }
      }
      return null;

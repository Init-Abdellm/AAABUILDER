...
class Tokenizer {
  ...
  tokenize() {
    ...
    while (this.position < this.content.length) {
      ...
      // Handle @ symbols for keywords like @agent and @end
      if (char === '@') {
      ...
      // Fallback: treat any other printable character as generic CHAR so prompts can include punctuation
...
class EnhancedAgentParser {
      ...
      if (this.check('IDENTIFIER')) {
        ...
        while (!this.isAtEnd() && !this.check('IDENTIFIER') && !this.checkAny(['STEPS', 'OUTPUTS', 'OUTPUT', 'AGENT_END'])) {
          if (this.check('TYPE')) {
            ...
            this.advance();
            this.expectToken('COLON');
            // Accept IDENTIFIER or keyword tokens like INPUT as values
            const fromToken = this.advance();
            variable.from = fromToken.value;
  ...
  parsePrompt() {
    if (this.check('PIPE')) {
      ...
      // Multi-line prompt
      let prompt = '';
      while (!this.isAtEnd() && !this.checkAny(['INPUT', 'OPERATION', 'WHEN', 'SAVE', 'KIND', 'TYPE', 'PROVIDER', 'MODEL', 'RETRIES', 'TIMEOUT_MS', 'OUTPUTS', 'OUTPUT', 'AGENT_END'])) {

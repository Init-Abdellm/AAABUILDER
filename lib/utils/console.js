...
// Store reference to original console to avoid circular dependency
...
`;
  ...
  // Clean, minimal header for operations
  operationHeader(operation, details = '') {
    originalConsole.log();
    originalConsole.log(chalk.magenta('●') + ' ' + chalk.white.bold(operation) + (details ? chalk.gray(' • ' + details) : ''));
  ...
  showWelcome() {
    originalConsole.log();
    originalConsole.log(chalk.magenta('🚀 Welcome to Zinebi - The Ultimate AI/ML Development Platform!'));
    originalConsole.log(chalk.gray('   Type "aaab --help" for available commands or "aaab show-capabilities" for AI/ML features'));
    originalConsole.log();
  }
  ...
  header(title, subtitle = '') {
    originalConsole.log();
    originalConsole.log(chalk.magenta('●') + ' ' + chalk.white.bold(title) + (subtitle ? chalk.gray(' • ' + subtitle) : ''));
    originalConsole.log(chalk.gray('─'.repeat(Math.min(title.length + (subtitle ? subtitle.length : 0) + 10, 60))));
    originalConsole.log();
  }
  ...
  section(title) {
    originalConsole.log();
    originalConsole.log(chalk.magenta('●') + ' ' + chalk.white.bold(title));
    originalConsole.log(chalk.gray('─'.repeat(Math.min(title.length + 10, 50))));
  }

  info(message, icon = 'ℹ', showTimestamp = true) {
    const timestamp = showTimestamp ? chalk.gray(` [${this.getTimestamp()}]`) : '';
    originalConsole.log(chalk.gray('  ') + chalk.cyan(icon) + ' ' + chalk.white(message) + timestamp);
  }
  ...
  success(message, icon = '✓') {
    ...
    originalConsole.log(chalk.gray('  ') + chalk.green(icon) + ' ' + chalk.white(message) + chalk.gray(` [${timestamp}]`));
  }

  warn(message, icon = '⚠') {
    const timestamp = this.getTimestamp();
    originalConsole.log(chalk.gray('  ') + chalk.yellow(icon) + ' ' + chalk.white(message) + chalk.gray(` [${timestamp}]`));
  }

  error(message, icon = '✗') {
    const timestamp = this.getTimestamp();
    originalConsole.log(chalk.gray('  ') + chalk.red(icon) + ' ' + chalk.white(message) + chalk.gray(` [${timestamp}]`));
  }
  ...
  step(stepNumber, stepName, status = 'running') {
    ...
    // Clean step display
    originalConsole.log(chalk.gray('  ') + colors[status](icons[status]) + ' ' +
  ...
  table(headers, rows) {
    ...
    const totalWidth = widths.reduce((sum, w) => sum + w, 0) + headers.length - 1;

    // Header
    originalConsole.log(chalk.gray('┌' + '─'.repeat(totalWidth) + '┐'));
    originalConsole.log(chalk.gray('│') + headers.map((header, i) =>
      chalk.white.bold(` ${header}`.padEnd(widths[i]))
    ).join(chalk.gray('│')) + chalk.gray('│'));
    originalConsole.log(chalk.gray('├' + widths.map(w => '─'.repeat(w)).join('┼') + '┤'));

    // Rows
    rows.forEach(row => {
      originalConsole.log(chalk.gray('│') + row.map((cell, i) =>
        chalk.white(` ${String(cell || '')}`.padEnd(widths[i]))
      ).join(chalk.gray('│')) + chalk.gray('│'));
    });
  ...
  json(obj, title = '') {
    if (title) {
      originalConsole.log(chalk.gray('  ') + chalk.cyan('📄') + ' ' + chalk.white.bold(title));
    }
    originalConsole.log();
    const jsonStr = JSON.stringify(obj, null, 2);
    jsonStr.split('\n').forEach(line => {
      originalConsole.log(chalk.gray('    ') + chalk.cyan(line));
    });
    originalConsole.log();
  }

  execution(agentName, version, input) {
    this.operationHeader(`Executing Agent: ${agentName}`, `Version ${version}`);
  ...
  summary(stats) {
    originalConsole.log();
    originalConsole.log(chalk.magenta('●') + ' ' + chalk.white.bold('Execution Summary'));
    originalConsole.log(chalk.gray('─'.repeat(40)));
    ...
    const summaryData = [
      ...
      ...Object.entries(stats).map(([key, value]) => {
        if (typeof value === 'boolean') {
          return [key, value ? chalk.green('✓ Yes') : chalk.red('✗ No')];
        }
    ...
    summaryData.forEach(([key, value]) => {
      originalConsole.log(chalk.gray('  ') + chalk.cyan(key + ':') + ' ' + chalk.white(value));
    });

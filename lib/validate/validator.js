...
class AgentValidator {
  ...
  performSemanticValidation(ast) {
    ...
    if (ast.output) {
      // Extract variable names from template strings like "{response}"
      const outputVariables = this.extractVariables(ast.output);
      if (outputVariables.length === 0) {
        // No template variables, check if output is a literal
        ...
        for (const variable of outputVariables) {
          const outputExists = this.checkOutputReference(ast, variable);
          if (!outputExists) {
            errors.push(`Output '${ast.output}' references non-existent variable or step result '${variable}'`);
          }
        }
      }
    }
    ...
    for (const step of ast.steps) {
      if (step.when) {
        ...
        for (const variable of variables) {
          ...
          if (!varExists) {
            errors.push(`Step '${step.id}' when condition '${step.when}' references non-existent variable '${variable}'`);
          }
        }
      }
  ...
  checkOutputReference(ast, variable) {
    // Check if it's a step output (saved result)
    const stepWithOutput = ast.steps.find(step => step.save === variable);
    if (stepWithOutput) {
      return true;
    }

    // Check if it's a step output in the outputs object
    const stepWithOutputs = ast.steps.find(step => step.outputs && step.outputs[variable]);
    if (stepWithOutputs) {
      return true;
    }
    ...
    if (ast.secrets && Array.isArray(ast.secrets) && ast.secrets.find(secret => secret.name === variable)) {
      return true;
    }
